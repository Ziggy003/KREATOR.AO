                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <Button 
                      ...
                      onClick={() => login(formData.type as any, "google")}
                    >
                    ...
                    <Button 
                      ...
                      variant="outline" 
                      className="h-12 gap-2"
                      onClick={() => login(formData.type as any, "facebook")}
                    >
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => login(formData.type as any, "google"), 1500); // Mocking Instagram login with Google for now
                    }}
                  >
                ...
                >
                    ...
                    {[1, 2, 3].map(i => (
                      <div 
                        ...
                        className={cn(
                          ...
                          step >= i ? "terra-gradient" : "bg-borda"
                        )}
                      />
                    ))}
                  </div>
                  ...
                  {step === 1 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                      ...
                      <Button onClick={handleNext} variant="gradient" className="w-full h-12 gap-2">
                  ...
                  {step === 2 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-muted">Nome Artístico / Empresa</label>
                        <Input 
                          placeholder="Ex: DJ Sassa" 
                          value={formData.name}
                          onChange={e => setFormData({...formData, name: e.target.value})}
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-muted">Nicho / Categoria</label>
                        <Input 
                          placeholder="Ex: Música, Tecnologia, Moda" 
                          value={formData.niche}
                          onChange={e => setFormData({...formData, niche: e.target.value})}
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-muted">Localização</label>
                        <select 
                          className="flex h-10 w-full rounded-input border border-borda bg-noite px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sol"
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button onClick={handleNext} variant="gradient" className="flex-[2] h-12 gap-2">
                          Próximo <ArrowRight className="w-4 h-4" />
                        </Button>
                      </div>
                    </motion.div>
                  ...
                  {step === 3 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 text-center">
                      <h4 className="font-bold mb-4">Conecta a tua primeira plataforma</h4>
                      <div className="grid grid-cols-2 gap-3 mb-6">
                        {[
                          { id: "tiktok", icon: Music2, label: "TikTok" },
                          { id: "youtube", icon: Youtube, label: "YouTube" },
                          { id: "instagram", icon: Instagram, label: "Instagram" },
                          { id: "spotify", icon: Globe, label: "Spotify" }
                        ].map(p => (
                          <button
                            key={p.id}
                            onClick={() => setFormData({...formData, platform: p.id})}
                            className={cn(
                              "flex items-center gap-3 p-4 rounded-button border transition-all",
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button onClick={handleSubmit} variant="gradient" className="flex-[2] h-12 gap-2">
                          Concluir Registo <CheckCircle2 className="w-4 h-4" />
                        </Button>
                      </div>
                      <button onClick={handleSubmit} className="text-xs text-muted hover:text-sol transition-colors">
                        Saltar esta etapa por agora
                      </button>
                    </motion.div>
                  )}
                </motion.div>
